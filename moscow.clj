...
(defn выделить-фракции [уик]
  (let [все-партии (партии уик)]
    (vec (cons (- (избиратели уик) (сумма все-партии)) все-партии))))

(doseq [уик (take 600 уики)]
  (when-let [смещение
             [(int (* размер-графика (window-weight явка уик окно-явки)))
              (int (* размер-графика (window-weight доля-ер уик окно-ер)))]]
    (нанести холст отступ размер-графика смещение шаблон (reverse (zip (выделить-фракции уик) цвета)))
